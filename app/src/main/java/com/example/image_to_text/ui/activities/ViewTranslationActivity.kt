...
class ViewTranslationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Utils.logAnalytic("ViewTranslation activity")
        ...
        if (filePath != null) {
            ...
            back.setOnClickListener {
                Utils.logAnalytic("ViewTranslation close button clicked")
                finish()
            }

            copy.setOnClickListener {
                Utils.logAnalytic("ViewTranslation copy button clicked")
                yourString?.let { it1 -> copyTextToClipboard(it1) }
            }

            save.setOnClickListener {
                Utils.logAnalytic("ViewTranslation save button clicked")
                yourString?.let { it1 -> saveImageAndTextToSQLite(applicationContext,bitmap, it1) }
            }

            share.setOnClickListener {
                Utils.logAnalytic("ViewTranslation share button clicked")
            ...
            textToSpeech = TextToSpeech(
                ...
                if (status != TextToSpeech.ERROR) {
                    ...
                    textToSpeech?.language = Locale(code.toString())
                } else {
                    Toast.makeText(
            ...
            speaker.setOnClickListener {
                Utils.logAnalytic("ViewTranslation speaker button clicked")
